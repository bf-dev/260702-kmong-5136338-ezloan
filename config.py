...
FRONTIER_POLL_SECONDS = 0.2   # look-ahead(프런티어 1글 체크) 주기 - 새 글 감지 지연 상한.
...
POST_ABSENT_GIVEUP_STREAK = 2000
...
POST_ABSENT_FAST_RETRY_CYCLES = 160   # 0.2s * 160 = 32s. 매 사이클 즉시 재시도(속도 유지 구간).
POST_ABSENT_BACKOFF_INTERVAL = 20     # 그 이후엔 20사이클(=4s)마다 한 번만 실제 재시도.
