...
# 2.6.2 (2026-08-23): 2.6.1 의 exe 는 네이버 로그인 버튼 셀렉터 수정(0f353c2) 이전에
# 빌드되어, 새 로그인이 필요한 순간 무조건 TimeoutException 으로 죽는다. 코드 변경 없이
# '현재 main 을 그대로 빌드'하기 위한 버전 번호다(로그인 수정 + 2.6.1 장애/세션 + 2.6.0 감지).
APP_VERSION = "2.6.2"
CUSTOMER_ID = "5136338"
...
# 새 글에서 'no permission' 이 나오면 곧바로 포기하지 않고 이 시간 동안 같은 글 번호를 붙잡고
# rq_addbanner_check 를 다시 두드린다. 근거(실측 2026-08-23, 글 32005): 이지론은 글 번호를
# 먼저 채번해 /rq/{id} 를 부분적으로 서빙하고, 배너 등록은 그보다 18.4초 뒤에 열렸다. 그 창의
# 'no permission' 은 계정 문제가 아니라 '아직 안 열림'이다. 예전엔 여기서 seen 처리 + 프런티어
# 전진이라 그 글을 영영 놓쳤다(32005 에 우리 배너 없음, 옥자대부는 개시 +0.14초에 등록).
# 이 창을 넘겨도 계속 거부되면 그때는 진짜 계정 측 거부로 보고 예전대로 포기한다.
NO_PERM_RETRY_SECONDS = 90.0
...
# '세션 만료'로 오판하고 등록 스레드를 종료해 17시간 57분을 놓쳤다. 이제는 사이트가
# 대답하지 않으면 '판정 불가'로 보고, 아래 간격으로 무한정 재시도하며 기다린다.
# 3초에서 시작해 2배씩 늘리고 30초에서 멈춘다. 상한을 30초로 잡은 이유: 이 상한이 곧
# '사이트가 살아난 뒤에도 앱이 놀고 있는 최대 시간' 이다. 이 프로그램의 값어치가 초 단위
# 선점이므로 60초는 너무 비싸고, 30초면 장애 중 사이트를 두드리는 부담(분당 2회)도 무시할
# 수 있는 수준이다.
SITE_RETRY_MIN_SECONDS = 3.0
SITE_RETRY_MAX_SECONDS = 30.0
# 원격 로그(works.insu.ng)로 '사이트 응답 없음' 을 다시 보고하는 최소 간격(초).
# 장애가 몇 시간 이어져도 로그 행이 수천 줄로 불어나지 않게 한다.
SITE_RETRY_REPORT_SECONDS = 300.0
# 진짜 로그아웃(200 응답이 로그아웃 상태를 보여줌)일 때 강제 재로그인을 시도하는 최대 횟수.
# 이걸 다 쓰고도 실패하면 그때만 등록 스레드를 멈춘다(distinct reason 으로 로그를 남긴다).
SESSION_RELOGIN_MAX_ATTEMPTS = 3
# 재로그인 시도 사이 간격(초). 크롬 창을 띄우고 네이버 로그인을 하는 무거운 경로라
# 빠르게 반복하지 않는다.
SESSION_RELOGIN_RETRY_SECONDS = 60.0

# --- 재시작 시 놓친 글 따라잡기 ------------------------------------------------
# 재시작 직전 창(프로그램이 꺼져 있던 동안)에 올라온 글은 예전엔 baseline 흡수 때문에
# 영구히 건너뛰어졌다(2026-08-05 실측: 31165~31174 등 10건 배너 누락).
# 이제는 '디스크에 저장된 seen 의 최대 글번호' 보다 새 글만 미처리로 남겨 등록을 시도한다.
# 다만 며칠 꺼져 있었던 경우까지 전부 따라잡으면 오래된 글에 배너 잔여를 낭비하므로,
# 따라잡는 개수는 가장 최근 것부터 이 개수까지로 제한한다.
RESTART_CATCHUP_MAX = 10

# --- 낡은(캐시된) 목록 방어 ----------------------------------------------------
# 2026-08-22 장애 실측: 원본이 521 로 죽자 Cloudflare 가 '오래전 캐시된 /rq 페이지'를
# HTTP 200 으로 대신 내려줬다(그 페이지 최신 글=30104, 당시 실제 프런티어=31985).
# 글번호는 단조증가하므로, 목록의 최신 번호가 이미 확인한 최신보다 이 값 이상 뒤처져
# 있으면 그건 '지금의 목록'이 아니다 -> 프런티어 되감기/목록 안전망/로그아웃 판정을 건너뛴다.
STALE_LIST_MAX_LAG = 50
# 다만 글이 실제로 대량 삭제돼 목록이 진짜로 줄어든 경우까지 영구히 무시하면 안 되므로,
# 같은 상태가 이 횟수(무거운 tick 기준, 1초당 1회 ≈ 10분) 이어지면 새 기준으로 받아들인다.
STALE_LIST_ACCEPT_AFTER = 600
